...
if __name__ == '__main__':
    ...
    argParser.add_argument('--scope_high', action='store', default=-0.15, type = float, help='High scope cut')
    argParser.add_argument('--toa_low', action='store', default=20, type = int, help='Low toa cut')
    argParser.add_argument('--toa_high', action='store', default=700, type = int, help='High toa cut')
    ...
    for config in run_configs:
        ...
        def load_events(chunk: list[str]):
            ...
            events = uproot.concatenate(chunk, branches)  
            ...
            peak_times, mcp_amplitude = mcp_fit.MCPSignalScaler.calc_mcp_peaks(nanoseconds, ak.to_numpy(events['mcp_volts']))
            ...
            events["mcp_timestamp"] = mcp_timestamp
            events["mcp_amplitude"] = mcp_amplitude
            events["clock_timestamp"] = clock_timestamp

            ## SELECT MODULE BASED ON CHIPID
            ...
            if is_telescope_setup: # (logic is correct here from above conditionals)
                chipid_sel = (events.chipid==chipid)
                events = ak.zip({
                    'mcp_timestamp': events.mcp_timestamp,
                    'mcp_amplitude': events.mcp_amplitude,
                    'clock_timestamp': events.clock_timestamp,
                    'row': events.row[chipid_sel],
                    'col': events.col[chipid_sel],
                    'toa_code': events.toa_code[chipid_sel],
                    'tot_code': events.tot_code[chipid_sel],
                    'cal_code': events.cal_code[chipid_sel], 
                    'nhits': events.cal_code[chipid_sel]
                })

                num_chipid_evs = len(ak.flatten(events.row))
                # if num_chipid_evs == 0:
                #     print(f"NO EVENTS WITH SELECTED MODULE ({module}) or CHIPID={module<<2}")
                #     exit()
                print(f"Number of events with chipid ({chipid}): {num_chipid_evs}/{len(events)}")
            return events
        ...
        presel = scope_sel & toa_sel & tot_sel & cal_sel #& amp_sel
        #---------------------------------------------------#
        ...
        if file_style=="DESY_April":
            ...
            #     all_chips = np.unique(events.chipid)
            #     for chipid in all_chips:
            #         presel = presel & (events.chipid == chipid)
        ...
        dt_axis       = hist.axis.Regular(500, 8, 20, name="dt", label="time")
