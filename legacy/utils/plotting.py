...
class TBplot:
    def __init__(self, data_dir: str, load_full_collection:bool=False):
        ...
        with open(os.path.join(data_dir, 'thresholds.json')) as f:
            ...
            self.thresholds = ThresholdsOld(**thresholds)
    ...
    @figuration(width=9,height=6,font_size=18)
    def histo1D(self, xVar:str, pix:tuple=None, xcuts:tuple=None, do_fit:bool=True, save_path:str=None):
        ...
        plt.legend()
        if save_path is not None:
            plot_saver(pix, pix_label, self.load_full_collection, save_path, xVar)
    
    @figuration(width=9,height=9,font_size=18)
    def heatmap(self, xVar:str, yVar:str, pix:tuple=None, xcuts:tuple=None, ycuts:tuple=None, save_path:str=None):
        """
        ...
        if save_path is not None:
            plot_saver(pix, pix_label, self.load_full_collection, save_path, xVar, yVar, 'heatmap_v2')

    @figuration()
    def timewalk_scatter(self, pix:tuple, save_path:str=None):
        model='cubicLM'
        row, col = pix
        ...
        plt.legend()
        if save_path is not None:
            plt.savefig(os.path.join(save_path, f'r{row}_c{col}_timewalk_scatter.png'))

    def get_field_vals(self, field:str, pix:tuple=None) -> akArray:
        """Retrieves a values from an awkward array by selecting the field."""
        data = self.events
        if pix is not None:
            data = ak.flatten(data[(data.row==pix[0]) & (data.col==pix[1])])
            return data[field]
        else:
